...
module.exports.addPost = async (req,res) => {
    ...
    await orginizationModel.findOneAndUpdate({_id}, { $addToSet: {posts: post._id} });
...
module.exports.getChannelPosts = async (req, res) => {
    ...
    let posts = postsArray.map(element => {
        ...
        element.time = time.slice(0,5) + time.slice(8,11) + ", "+ date.toLocaleDateString().replace(/\//g, '.');
        delete element.updatedAt;
        let  mediaFile = fs.readFileSync((__dirname.substring(0, __dirname.length-8) + 'mediaFiles\\' + element.mediaFile));
